...
export default function Home() {
  ...
  return <main>
    ...
    <div className="shell">
      <nav className="nav"><a className="logo" href="#top"><span className="logo-mark">AJ</span> Ahmad Jawad</a><div className="nav-links"><a href="#about">About</a><a href="#proof">Principles</a><a href="#work">Selected work</a><a href="#contact">Contact</a></div><a className="button" href="https://github.com/ahmedjawad24" target="_blank" rel="noreferrer">GitHub <ArrowUpRight size={15} /></a></nav>
      <section className="hero" id="top"><div className="hero-gallery" aria-hidden="true"><span className="hero-image image-one" /><span className="hero-image image-two" /><span className="hero-image image-three" /><span className="hero-image image-four" /><div className="hero-shade" /></div><div className="hero-copy-block"><div className="kicker">Applied AI & MLOps Engineer · Open to opportunities</div><h1>AI products that teams can <em>understand, trust, and use.</em></h1><p className="hero-copy">I build production-focused systems where model outputs are not just accurate, but legible to people making real decisions.</p><div className="actions"><a className="button primary" href="#work">Explore the work <ArrowUpRight size={16} /></a><button className="button" onClick={() => setChatOpen(true)}>Ask my assistant <Bot size={16} /></button></div><div className="hero-ribbon" aria-hidden="true"><span>GENAI</span><span>AGENTS</span><span>MLOPS</span><span>EXPLAINABILITY</span><span>TRUST LAYER</span><span>PRODUCT DELIVERY</span></div><div className="hero-stats"><span><strong>09</strong> shipped systems</span><span><strong>03</strong> private case studies</span><span><strong>01</strong> principle: clarity first</span></div></div><div className="signal"><div className="signal-grid" /><div className="signal-orbit orbit-one" /><div className="signal-orbit orbit-two" /><div className="signal-scan" /><div className="signal-content"><div className="eyebrow">System status / 2026</div><h2>Built for clarity from first output to final decision.</h2><div className="signal-list"><span>Traceable model behavior</span><span>Human review checkpoints</span><span>Production-ready delivery</span></div><div className="signal-foot"><span><span className="pulse">●</span> Available for meaningful problems</span><span>Open globally</span></div></div></div></section>
      <section className="section" id="about"><div className="section-head"><div><div className="kicker">01 / About me</div><h2>Engineer first.<br />AI by purpose.</h2></div><p className="section-intro">I focus on making advanced models understandable, auditable, and useful in production contexts where trust matters.</p></div><div className="about"><p className="about-copy">Curious about the edge where <strong>AI research</strong> becomes a product people can rely on.</p><div className="about-details"><p>I&apos;m Ahmad Jawad, an engineer working across generative AI, machine learning, and production-minded software. My projects range from AI accountability on Solana to clinical explainability and fraud operations.</p><div className="education"><span className="eyebrow">Education</span><strong>BS Computer Science</strong><span>Pak Austria Fachhochschule: Institute of Applied Sciences & Technology</span></div><p>I&apos;m especially interested in teams solving difficult problems with high standards for experimentation, reliability, and responsible deployment.</p><div className="actions"><a className="button" href="https://github.com/ahmedjawad24" target="_blank" rel="noreferrer"><Github size={15} /> GitHub profile</a><a className="button" href="https://www.linkedin.com/in/ahmad-jawad-248870267" target="_blank" rel="noreferrer"><Linkedin size={15} /> LinkedIn</a></div></div></div></section>
      <section className="section proof" id="proof"><div className="section-head"><div><div className="kicker">02 / Why this works</div><h2>Clarity is a feature,<br />not a side effect.</h2></div><p className="section-intro">I design model products so engineering, operations, and leadership can all read the same system with confidence.</p></div><div className="proof-grid"><article className="proof-card"><h3>Decision-ready outputs</h3><p>Results are framed in language teams can act on, not just benchmark numbers.</p></article><article className="proof-card"><h3>Measured reliability</h3><p>Evaluation and thresholds are chosen for real-world risk, not only offline performance.</p></article><article className="proof-card"><h3>Human oversight built-in</h3><p>Critical workflows include handoff points and explainability to prevent blind automation.</p></article></div></section>
      <section className="section" id="work"><div className="section-head"><div><div className="kicker">03 / Selected systems</div><h2>Work with a point<br />of view.</h2></div><p className="section-intro">Tap a project to open its signal brief. Each system is designed to make a hard problem clearer, more useful, and easier to trust.</p></div><div className="filters">{filters.map((item) => <button className={`filter ${filter === item ? "active" : ""}`} key={item} onClick={() => setFilter(item)}>{item}</button>)}</div><div className="project-scroll"><div className="project-grid">{visible.map((project, index) => <button className={`project ${project.featured ? "featured" : ""}`} onClick={() => setSelectedProject(project)} key={project.title}><div className="project-top"><span className="project-index">0{index + 1} / {project.type}</span>{project.private ? <span className="private-label">Private</span> : <ArrowUpRight className="project-link" size={20} />}</div><div className="project-visual"><span>{project.signal}</span><span className="visual-line" /></div><div><h3>{project.title}</h3><p>{project.description}</p></div><div className="project-tags">{project.tags.map((tag) => <span className="tag" key={tag}>{tag}</span>)}</div><span className="project-cta">Open brief</span></button>)}</div></div></section>
      <section className="section" id="approach"><div className="section-head"><div><div className="kicker">04 / How I work</div><h2>Useful beats<br />impressive.</h2></div><p className="section-intro">The strongest AI products respect both the machine and the person relying on it.</p></div><div className="capabilities"><div className="capability"><BrainCircuit size={22} /><h3>Build the intelligence</h3><p>Model selection, prompting, agent design, retrieval, and evaluation shaped around the real problem.</p></div><div className="capability"><Workflow size={22} /><h3>Operationalize the system</h3><p>Reproducible pipelines, versioned artifacts, APIs, dashboards, and deployment paths that survive contact with reality.</p></div><div className="capability"><ShieldCheck size={22} /><h3>Earn confidence</h3><p>Transparent metrics, explainability, human review, and explicit limits built into the experience.</p></div></div></section>
